...
# 主智能体采用延迟初始化，首次提交任务时才创建（见 agent/main_agent.py）
from agent.main_agent import run_deep_agent
from api.monitor import manager

@asynccontextmanager
async def lifespan(_app: FastAPI):
    """服务启动时绑定事件循环，供后台线程通过 run_coroutine_threadsafe 推送 WebSocket 消息。"""
...
app = FastAPI(title="Deep Search Pro API", lifespan=lifespan)
...
async def upload_files(files: List[UploadFile] = File(...), thread_id: str = Form(...)):
    ...
    # 2. [保存] 遍历并写入文件
    for file in files:
        # 防止路径穿越：只取文件名部分，丢弃可能携带的目录信息
        safe_name = Path(file.filename).name or f"upload_{uuid.uuid4().hex}"
...
async def websocket_endpoint(websocket: WebSocket, thread_id: str):
    """WebSocket 实时通道：绑定 thread_id，接收心跳并保持连接活跃。"""
