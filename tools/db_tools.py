...
def get_table_data(table_name)->str:
    ...
    # 安全校验：表名只允许字母 / 数字 / 下划线，防止 SQL 注入
    if not re.fullmatch(r"[A-Za-z0-9_]+", table_name or ""):
        return f"非法表名：{table_name}，只允许字母、数字和下划线。"
...
def execute_sql_query(query)->str:
    ...
    # 只读防护：仅允许查询类语句，禁止写操作与多语句注入
    normalized = (query or "").strip().rstrip(";").strip().lower()
    if not normalized.startswith(("select", "show", "describe", "desc", "explain", "with")):
        return "已拒绝执行：仅允许只读查询（SELECT / SHOW / DESCRIBE / EXPLAIN / WITH）"
    if ";" in normalized:
        return "已拒绝执行：不允许一次执行多条 SQL 语句"
